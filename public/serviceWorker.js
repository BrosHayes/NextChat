const NEXTCHAT_CACHE = "nextchat-cache";
const NEXTCHAT_FILE_CACHE = "nextchat-file-cache";
...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    caches.open(NEXTCHAT_CACHE).then(function (cache) {
      return cache.addAll([]);
    }),
  );
});
...
async function upload(request, url) {
  ...
  const cache = await caches.open(NEXTCHAT_FILE_CACHE)
